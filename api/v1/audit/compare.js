...
var UCP_AGENT_PROFILE = "https://shopify.dev/ucp/agent-profiles/examples/2026-08-25/valid-with-capabilities.json";
var FETCH_MS = 12e3;
...
async function ucpRpc(endpoint, method, params) {
...
function gtinFromProducts(products) {
  let count = 0;
  let withGtin = 0;
...
async function probeUcpCatalog(storeUrl) {
  ...
  if (!listed.ok) {
    return {
      ...
      gtinPct: 0,
      error: listed.error,
      httpStatus: listed.status || void 0
    };
  }
  ...
  if (!searched.ok) {
    return {
      ...
      httpStatus: searched.status || void 0
    };
  }
  const products = searched.data.result?.structuredContent?.products ?? [];
  const { count, withGtin } = gtinFromProducts(products);
  const productCount = count > 0 ? count : searched.data.result?.structuredContent?.pagination?.total_count ?? products.length;
  return {
    available: true,
    endpoint,
    tools,
...
function ucpToCompareSnapshot(probe) {
  return {
...
function snapshot(label, _merchant, products, audit, summary) {
  return {
    source: audit.source,
    label,
...
function buildAuditCompare(url, crawl, oauth, ucp) {
  ...
  if (oauth) {
    ...
    headline = crawlSnap.gtinPct === 0 ? `Public crawl: 0% GTIN. UCP not on this store (${ucp.error ?? "no endpoint"}). Connect OAuth for Admin catalog.` : `Public crawl: ${crawlSnap.gtinPct}% GTIN. UCP endpoint not live yet.`;
  } else {
    headline = crawlSnap.gtinPct === 0 ? "Public feed has 0% GTIN \u2014 connect Shopify OAuth to compare Admin catalog." : `Public crawl: ${crawlSnap.gtinPct}% GTIN, ${crawlSnap.catalogScore}/${crawlSnap.catalogBudget} catalog pts.`;
  }
  return {
    ok: true,
    url,
    shop: oauth?.shop,
    crawl: crawlSnap,
    ucp: ucp ? ucpToCompareSnapshot(ucp) : void 0,
    oauth: oauthSnap,
    delta,
    headline
  };
...
async function fetchText(url, accept = "text/html,application/json,text/plain,*/*") {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), FETCH_MS2);
  try {
    const res = await fetch(url, {
      signal: controller.signal,
      headers: {
...
async function getClientCredentialsToken(shop) {
...
function getCachedOAuthToken(shop) {
  const entry = tokenCache.get(`oauth:${normalizeShop(shop)}`);
  if (!entry || entry.expiresAt <= Date.now()) return null;
  return entry.accessToken;
}
async function fetchAdminProducts(shop, accessToken) {
  const domain = normalizeShop(shop);
  const products = [];
  let nextUrl = `https://${domain}/admin/api/2025-01/products.json?limit=50&fields=id,title,body_html,vendor,product_type,tags,variants`;
  while (nextUrl) {
...
function adminToCatalogExport(shop, products) {
  return {
    exported_at: (/* @__PURE__ */ new Date()).toISOString(),
...
async function syncShopifyStore(shop) {
  const domain = normalizeShop(shop);
  ...
  if (!token) {
    return {
      ok: false,
      error: "No access token \u2014 install the app on this shop (OAuth) or use a dev store in your Partner org (client credentials)."
    };
  }
...
// api/v1/audit/compare.ts
async function handler(req, res) {
  if (req.method !== "POST") {
    res.setHeader("Allow", "POST");
    return res.status(405).json({ error: "Method not allowed" });
  }
  const rl = await checkRateLimitAsync(`audit-compare:${clientIp(req)}`, 15, 60 * 60 * 1e3);
  if (!rl.allowed) {
