...
| Product records an agent can read | **6** | "Ambiguous page structure — **6%**" | We read back the JSON-LD the page actually emits (`emittedProductRecords` walks the output of `catalogJsonLd`, the same function `ShopView` writes into `<script type="application/ld+json">`) and require each record to carry `name`, `sku`, a resolvable `gtin13`, and an `Offer` with `price`, `priceCurrency` and `availability`. **The field list is ours** and it is the whole definition — the source gives this row no prose anywhere on the page. A store-local SKU resolves to nothing for an agent that has never seen the store; a GTIN does. Context: only **19%** of Product schemas carry an Offer object at all (Digital Applied, 5,000 sites). **Stated limit (added 2026-08-31, wave 5):** on a type-conforming catalog `catalogJsonLd` always emits `name`, `sku` and all three `Offer` fields, so on the two shipped fixtures **`gtin13` is the only required field a record can be missing** — this line is a GTIN-coverage test read out of the emitted document, and the six-field list only starts to bite on an imported catalog (a Shopify row with an empty title lands as `name: ''`). The proof is the tamper's own output: drop `gtin13` from the list and both stores print a perfect 6/6. Pinned by an assertion in `scripts/verify-readiness.mjs`. |
