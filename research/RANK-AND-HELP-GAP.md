...
4. **Offer/schema line measured on crawl path** — **DONE** (2026-08-31): `offerPct` on crawl (`src/lib/offer-crawl.ts`); RankingsPanel Offer column; `R7-offer-on-crawl.md`.
5. **ACP eligibility smoke (non-upload)** — **DONE** (2026-08-31): `policySmoke` on `POST /api/v1/audit/url` (`src/lib/policy-smoke.ts`); honest `measured: false` when no URLs; no `is_eligible_*` without partner feed.
...
Until **E1 OAuth pairs** + live checkout probe: **do not** claim full holistic rank+help. Gaps 1, 3, 4, 5, 6 (label), 7 (help) are closed.
