...
async function main() {
  ...
  for (const r of rows) {
    ...
    const sig = [r.captchaHint ? 'captcha?' : '', r.gtinPct < 50 ? 'low-gtin' : '', r.offerPct != null && r.offerPct < 20 ? 'low-offer' : ''].filter(Boolean).join(',') || '—';
    const offer = r.offerPct != null ? `${r.offerPct}%` : '—';
    console.log(
      `| ${r.url.replace(/^https?:\/\/(www\.)?/, '').slice(0, 22)} | ${r.method} | ${r.products} | ${r.gtinPct}% | ${offer} | ${r.catalogScore}/${r.catalogBudget} | ${r.sandboxScore}/100 | ${sig} |`,
    );
  }
